...
def schnorr_verify(msg: bytes, pubkey: bytes, sig: bytes) -> bool:
    ...
    if (P is None) or (r >= p) or (s >= n):
        return False
    print("concat=", hex(int_from_bytes(sig[0:32] + pubkey + msg)))
...
def apply_tweak(keyagg_ctx: KeyAggContext, tweak: bytes, is_xonly: bool) -> KeyAggContext:
    ...
    gacc_ = g * gacc % n
    tacc_ = (t + g * tacc) % n
    #print("Q, gacc, tacc=", Q_, gacc_, tacc_);
...
def key_agg_and_tweak(pubkeys: List[PlainPk], tweaks: List[bytes], is_xonly: List[bool]) -> KeyAggContext:
    ...
    #print("@@keyagg pubkeys:", keyagg_ctx)

    v = len(tweaks)
    for i in range(v):
        keyagg_ctx = apply_tweak(keyagg_ctx, tweaks[i], is_xonly[i])
        #print("@@keyaggctx : ",hex(keyagg_ctx[0][0]));
...
def get_session_values(session_ctx: SessionContext) -> Tuple[Point, int, int, int, Point, int]:
    ...
    #print("\n--------------\n-----------session values input :", aggnonce, pubkeys, tweaks, is_xonly, msg )
    Q, gacc, tacc = key_agg_and_tweak(pubkeys, tweaks, is_xonly)
    #print("Qx, gacc, tacc", hex(Q[0]), gacc, tacc);
    ...
    #print("-----------session values output :", Q, gacc, tacc, b, R, e )
...
def test_sig_agg_vectors() -> None:
    ...
    for test_case in valid_test_cases:
        ...
        #print("\n   psig1=", hex(int_from_bytes(psigs[0])));
       
        sig = partial_sig_agg(psigs, session_ctx)
        #print("\n sig", hex(int_from_bytes(sig)));
